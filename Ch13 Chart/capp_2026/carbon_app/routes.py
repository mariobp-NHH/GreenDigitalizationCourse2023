...
import json
from capp.carbon_app.table_functions import get_entries, format_entries_for_table
from capp.carbon_app.table_functions import debug_raw_entries
from capp.carbon_app.charts_functions import get_emissions_by_transport, format_emissions_for_chart, debug_chart_data
...
def your_data():
    #Table
    table_raw_entries = get_entries(current_user)
    table_formatted_entries = format_entries_for_table(table_raw_entries)
    table_raw_debug = debug_raw_entries(table_raw_entries)
    
    
    #Charts.
    raw_emissions = get_emissions_by_transport(current_user)
    labels, emission_data = format_emissions_for_chart(raw_emissions)

    debug_chart_data(raw_emissions, labels, emission_data)
    

    return render_template('carbon_app/your_data.html', title='your_data', 
        table_raw_entries=table_raw_entries,
        table_formatted_entries=table_formatted_entries,
        table_raw_debug=table_raw_debug,
